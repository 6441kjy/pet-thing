...
public class BoardTools {
	...
	public static String getNavi(int totalPage, int rowsPerPage, String url, int pageNumber) {
		...
		if(pageNumber>1)result+=("<a href="+url+(pageNumber-1)+ " class=\"link-dark text-decoration-none\" >[이전]</a>");
		for(int i=1;i<=blockCnt;i++){
			if(pageNumber==i)	result+=("<b>");
			result+=("<a href="+url+i+ " class=\"link-dark text-decoration-none\">["+i+"]</a>");
			if(pageNumber==i)	result+=("</b>");
		}
		if(pageNumber<blockCnt)result+=("<a href="+url+(pageNumber+1)+ " class=\"link-dark text-decoration-none\">[다음]</a>");
	...
	public static String getSearch(List<String> lst, String jsFuncName) {
		...
		tag += "<input type=text name='searchWord'/>";
		tag += "<input type=button onclick="+jsFuncName;
		tag += " name='searchBtn' value='�˻�' style='width: 80px; '/>";
