...
<body style="width:100%; height:100%;">
		...
		<div id="lostImg" class="ps-2"
			style="width: 30%; height: 400px; float:left;">
			<div class="mt-1 text-center" style="height: 400px; line-height: 400px">선택하신 좌표의 동물 사진이 표시됩니다.</div>
		</div>
		...
		function setMarker() {
			...
			var positionsLost = [
				<% for(LostPetDTO lDTO : lostLst) {%>
					...
					imgPath : '<%=request.getContextPath()+"/img/img"+lDTO.getNo()+".png"%>'
					},
				<%}%>
			...
			for (var i = 0; i < positionsLost.length; i ++) {
			    ...
			    //----------------------------------
			    var lostImgDiv = document.getElementById('lostImg'); 
			    var lostImgMsg = positionsLost[i].imgPath;
			    //console.log(lostImgMsg+"?");
			    //console.log(Object.values(markersLost)+"!!!");
			    //console.log()
			    
			    
			    kakao.maps.event.addListener(markersLost, 'click', function() {
			        // 마커 위에 인포윈도우를 표시합니다
			        console.log(markersLost+"이건가");
			        lostImgDiv.innerHTML = "<img src="+lostImgMsg+" height='100%'>";});
			 
			   
			   //showImg(markersLost, lostImgMsg, lostImgDiv))
			}
			...
			//--------------------------------------
			function showImg(markersLost, lostImgMsg, lostImgDiv) {
				console.log(markersLost);
				lostImgDiv.innerHTML = "<img src="+lostImgMsg+" width='100%'>";
			}
